...
LEHM_roots = np.array([root(lambda t: emg(t, h, μ, σ, τ) - emg_mode(h, μ, σ, τ)[1]/2, -0.1).x for τ in τs]).squeeze()
IPLE_roots = np.array([root(lambda t: σ/τ + (t - μ)/σ - (σ/τ)**2*np.sqrt(π/2) * erf_lib.erfcx(1/np.sqrt(2)*(σ/τ - (t[0] - μ)/σ)), -0.1).x for τ in τs]).squeeze()
